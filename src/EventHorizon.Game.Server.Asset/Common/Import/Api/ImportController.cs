    ...
    : Controller
{
    ...
    [ProducesResponseType(typeof(UploadImportFileModel), StatusCodes.Status201Created)]
    ...
    )
    {
        if (file is null)
        {
            return BadRequest(
                new ErrorDetails
                {
                    ...
                    ErrorCode = CommonImportErrorCodes.COMMON_IMPORT_MISSING_API_FILE_ARGUMENT,
                }
            );
        }
        ...
        return Created(
            ...
            new UploadImportFileModel(
                result.Result
            )
        );
    }

    private class UploadImportFileModel
    {
        public string Service { get; }
        public string Path { get; }

        public UploadImportFileModel(
            RunUploadOfServiceImportFileResult result
        )
        {
